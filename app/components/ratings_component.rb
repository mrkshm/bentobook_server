...
class RatingsComponent < ViewComponent::Base
    def initialize(rating: nil, form: nil, readonly: false)
    ...
    def render_readonly_stars
      content_tag(:div, class: "flex items-center") do
        (1..5).map do |i|
          star_svg(i <= @rating ? 'text-yellow-400' : 'text-gray-300')
        end.join.html_safe
      end
    end
    ...
    def star_classes(index)
      classes = "size-5 inline-flex justify-center items-center text-2xl rounded-full"
      classes += " #{@rating && index <= @rating ? 'text-yellow-400' : 'text-gray-300'}"
      classes
    end
  
    def star_svg(color_class = nil)
      content_tag(:svg, class: "shrink-0 size-5 #{color_class}", xmlns: "http://www.w3.org/2000/svg", width: "16", height: "16", fill: "currentColor", viewBox: "0 0 16 16") do
